...
#define      OV7725_DATA_2_GPIO_PIN                   GPIO_Pin_5
...
#define READ_FIFO_PIXEL(RGB565)   	do{\
	                                  RGB565=0;\
	                                  FIFO_RCLK_L();\
	                                  RGB565 = ((OV7725_DATA_GPIO_PORT->IDR & 1)<<8 | (OV7725_DATA_GPIO_PORT->IDR & 1<<1)<<8 | (OV7725_DATA_GPIO_PORT->IDR & 1<<5)<<5 | (OV7725_DATA_GPIO_PORT->IDR & 1<<7)<<4) | ((OV7725_DATA_GPIO_PORT->IDR) & 0xf000);\
	                                  FIFO_RCLK_H();\
                                    FIFO_RCLK_L();\
	                                  RGB565 |= ((OV7725_DATA_GPIO_PORT->IDR & 1) | (OV7725_DATA_GPIO_PORT->IDR & 1<<1) | (OV7725_DATA_GPIO_PORT->IDR & 1<<5)>>3 | (OV7725_DATA_GPIO_PORT->IDR & 1<<7)>>4) | (((OV7725_DATA_GPIO_PORT->IDR) & 0xf000)>>8);\
	                                  FIFO_RCLK_H();\
                                    }while(0)
...
void ImagDisp(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height, int num);
